        ...
        tableBody.innerHTML += `
            <tr>
                ...
                <td onclick="openProductView(${index})" style="cursor:pointer;">
                    ...
                    <small>${item.qty || ''} | ${item.weight || ''}</small>
                </td>
                <td style="font-size:11px; color:#28a745; font-weight:bold;">
                    P: ₱${(item.pricePiece || 0).toLocaleString()}<br>
                    B: ₱${(item.priceBox || 0).toLocaleString()}
                </td>
                <td>
...
function applyAdminUI() {
    ...
    document.getElementById('adminStatus').innerText = "Admin Mode Activated"; // Update text
    document.getElementById('adminStatus').style.color = "#2ecc71"; // Magiging green para halata
...
function copyNewJSON() { navigator.clipboard.writeText(JSON.stringify(currentLiveItems, null, 2)).then(() => alert("JSON Copied!")); }
