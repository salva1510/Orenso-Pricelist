        ...
        grid.innerHTML += `
            <div class="product-card">
                ...
                <div style="display:flex; flex-direction:column; gap:5px;">
                    ${isAdmin ? `<button onclick="deleteItem(${index})" style="background:#dc3545; color:white; padding:8px; border-radius:5px;">Delete</button>` : 
                    `
...
function openProductView(index) {
    ...
    document.getElementById('viewPriceContainer').innerHTML = `
        <button class="opt-btn" onclick="addToBasket('${item.name}', ${item.pricePiece}, 'Piece'); closeProductView();" style="width:48%; padding:12px;">+ Piece (₱${item.pricePiece})</button>
        <button class="opt-btn" onclick="addToBasket('${item.name}', ${item.priceBox}, 'Box'); closeProductView();" style="width:48%; padding:12px; background:#28a745;">+ Box (₱${item.priceBox})</button>
    `;
    ...
    for (let key in basket) {
        ...
        html += `<div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid #eee;">
            ...
            <div style="display:flex; align-items:center;">
                <button onclick="removeFromBasket('${key}')" style="width:30px; height:30px; border-radius:50%; border:none; background:#eee;">-</button>
                <span style="width:30px; text-align:center;">${item.count}</span>
                <button onclick="addToBasket('${item.originalName}', ${item.price}, '${item.type}'); renderBasket();" style="width:30px; height:30px; border-radius:50%; border:none; background:#eee;">+</button>
                <span style="margin-left:10px; font-weight:bold; width:75px; text-align:right;">₱${subtotal.toLocaleString()}</span>
            </div>
        </div>`;
    }
    list.innerHTML = html || "<p style='text-align:center; color:#999; padding:20px;'>Empty basket.</p>";
