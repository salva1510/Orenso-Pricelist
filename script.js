            ...
            <div class="product-card">
                ...
                <div onclick="openProductView(${index})" style="cursor:pointer; flex-grow:1;">
                    ...
                    <div class="item-details-text">
                        ${item.qty || ''} ${item.weight ? ' • ' + item.weight : ''}
                    </div>
                    <div class="price-tag">₱${(item.pricePiece || 0).toLocaleString()} / pc</div>
                </div>
                <div style="display:flex; flex-direction:column;">
                    ${isAdmin ? `<button onclick="deleteItem(${index})" style="background:#dc3545; color:#fff; border:none; padding:8px; border-radius:8px;">Delete</button>` : 
                    `
...
function openProductView(index) {
    ...
    document.getElementById('viewPriceContainer').innerHTML = `
        <button class="opt-btn btn-pc" onclick="addToBasket('${item.name}', ${item.pricePiece}, 'Piece'); closeProductView();" style="width:48%; padding:12px;">Add Piece (₱${item.pricePiece})</button>
        <button class="opt-btn btn-box" onclick="addToBasket('${item.name}', ${item.priceBox}, 'Box'); closeProductView();" style="width:48%; padding:12px;">Add Box (₱${item.priceBox})</button>
    `;
...
function sendOrder(platform) {
    ...
    if(!name || !address || Object.keys(basket).length === 0) return alert("Paki-puno ang pangalan at address!");
    ...
    else { navigator.clipboard.writeText(msg); alert("Copied! I-paste sa Messenger."); window.open(`https://m.me/${FB_USERNAME}`); }
}
...
function deleteItem(index) { if(confirm("Burahin ito?")) { currentLiveItems.splice(index, 1); displayItems(currentLiveItems); copyNewJSON(); } }
