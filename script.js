...
// THE REST OF YOUR FUNCTIONS (RETAINED)
...
function renderBasket() {
    ...
    for (let key in basket) {
        ...
        html += `<div style="display:flex; justify-content:space-between; padding:10px 0; border-bottom:1px solid #eee;">
            <span>${key} x ${item.count}</span>
            <span>₱${(item.price * item.count).toLocaleString()}</span>
        </div>`;
    }
    list.innerHTML = html || "<p>Empty basket.</p>";
...
function sendOrder(platform) {
    ...
    else { navigator.clipboard.writeText(msg); alert("Copied!"); window.open(`https://m.me/${FB_USERNAME}`); }
}
