...
// BAGONG UPDATE: May controls na para sa + at - sa loob ng Basket
function renderBasket() {
    ...
    list.innerHTML = html || "<p style='text-align:center; padding:20px; color:#888;'>Empty basket.</p>";
    document.getElementById('basketTotal').innerText = "₱" + total.toLocaleString();
}

// Function para dagdagan o bawasan ang quantity
...
// Function para tuluyang burahin ang item sa basket
function removeFromBasket(key) {
    if (confirm("Alisin ang item na ito sa basket?")) {
        delete basket[key];
        updateBasketCount();
        renderBasket();
    }
...
function sendOrder(platform) {
    ...
    else { navigator.clipboard.writeText(msg); alert("Order details copied to clipboard!"); window.open(`https://m.me/${FB_USERNAME}`); }
}
